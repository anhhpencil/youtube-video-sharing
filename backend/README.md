...
const getUser = async (userId) => {
  const user = await UserStaking.findById(userId);
  if (!user) {
...
const userStakingSchema = mongoose.Schema(
  {
...
userStakingSchema.plugin(toJSON);
userStakingSchema.plugin(paginate);

const UserStaking = mongoose.model('UserStaking', userSchema);
```
